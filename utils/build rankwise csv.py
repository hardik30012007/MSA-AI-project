...
def main():
    ...
    ap.add_argument("--input-dir", required=True, help="Folder with processed CSVs")
    ap.add_argument("--output-file", required=True, help="Final rankwise CSV")
    ap.add_argument(
        "--algorithms",
        default="IMSA,MSA,PSO,GWO,WOA,SSA,DE,GA,HHO",
        help="Comma-separated algorithm order",
    )
    ap.add_argument("--row-label", default="Function", help="Function or Problem")
    args = ap.parse_args()
    ...
    for algo in algo_order:
        file_path = input_dir / f"{algo}.csv"
        if not file_path.exists():
            raise FileNotFoundError(f"Missing file: {file_path}")
        algo_data[algo] = load_processed_csv(file_path, args.row_label)
    ...
    for label in labels:
        ...
        # Lower mean = better rank, stable tie-break by algo order
    ...
    columns = ["algo"]
    for algo in algo_order:
        columns.extend([f"{algo}.MEAN", f"{algo}.STD", f"{algo}.RANK"])
