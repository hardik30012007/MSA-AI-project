...
def process_one_csv(input_path: Path, output_dir: Path, row_label: str):
    ...
    out = pd.DataFrame(
        {
            row_label: df[first_col].astype(str),
            "Mean": run_data.mean(axis=1),
            "Std": run_data.std(axis=1, ddof=1),
        }
    )
...
def main():
    ...
    ap.add_argument("--input-dir", required=True, help="Folder containing raw CSVs")
    ap.add_argument("--output-dir", required=True, help="Folder to save processed CSVs")
