...
def main():
    ...
    for algo in algo_order:
        if algo == args.reference:
            continue
        summary.append({'Algorithm': algo, 'Win': int((out_df[f'{args.reference}_vs_{algo}_result'] == 'Win').sum()), 'Loss': int((out_df[f'{args.reference}_vs_{algo}_result'] == 'Loss').sum()), 'Tie': int((out_df[f'{args.reference}_vs_{algo}_result'] == 'Tie').sum())})
    pd.DataFrame(summary).to_csv(output_dir / f'wilcoxon_summary_{args.reference}.csv', index=False)
